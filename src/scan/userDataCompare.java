	...
	private void lineCompareJob(line l) throws Exception
		...
		for(int j=0; j<myUSync.getUserSyncTemplate().size(); j++)
			...
			if(myUSync.getUserSyncTemplate().get(j).getName().equals(pt))
				{
				/**
				 * SIP device like DECT have no "line text label". So
				 * We removed it to avoid stupid axl error during 
				 * correction process 
				 */
				if(l.getMyDevice().getModel().contains("SIP"))
					{
					variables.getLogger().debug("Line Text Label treatment : A SIP device has been found "+l.getMyDevice().getModel()+", it will be ignored");
					}
				else
					{
		...
		if(!methodesUtiles.isExceptionWord(currentData, myUSync.getTaskIndex()))
			...
			if(!pt.equals(patternType.lineexternalphonenumbermask))
				...
				if(isTooLong(newData))//We test if value is too long
					{
					//We check if auto correct is enable
					if(methodesUtiles.getTargetTask("checkfortoolongvalue", myUSync.getTaskIndex()).equals("true"))
						{
			...
			{
			variables.getLogger().debug("Exception word found in : "+currentData);
			}
		}
